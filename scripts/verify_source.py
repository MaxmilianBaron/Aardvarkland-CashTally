...
def check_prototype() -> None:
    ...
    require(match is not None, "Prototype script not found")
    node = shutil.which("node")
    if match and node:
        # `node --check -` can wait forever for stdin on Windows. A closed
        # temporary file behaves consistently on every supported host.
        with tempfile.TemporaryDirectory(prefix="vycetka-js-check-") as temp_dir:
            script_path = Path(temp_dir) / "prototype.js"
            script_path.write_text(match.group(1), encoding="utf-8")
            run([node, "--check", str(script_path)])
    elif not node:
        NOTES.append("node unavailable; prototype JavaScript syntax skipped")
